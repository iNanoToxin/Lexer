...
std::shared_ptr<AstNode> Parser::getExpression(const int p_Precedence) {
    ...
    {
        expression = getRhsExpression(p_Precedence, function_definition);
    }
    else if (const std::shared_ptr<AstNode> prefix_expression = getPrefixExpression())
    {
        expression = getRhsExpression(p_Precedence, prefix_expression);
    }
    else if (const std::shared_ptr<AstNode> primary_expression = getPrimaryExpression())
    {
        expression = getRhsExpression(p_Precedence, primary_expression);
    }
